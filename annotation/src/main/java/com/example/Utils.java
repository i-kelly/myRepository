...
public class Utils {
    ...
    public static boolean isValidClass(Messager messager,
                                       TypeElement element) {
        ...
        if (!isPublic(element)) {
            ...
            messager.printMessage(Diagnostic.Kind.ERROR, message, element); return false;
        }
    ...
    public static String getPackageName(Elements elements,
                                        TypeElement typeElement)
            throws NoPackageNameException {
        PackageElement pkg = elements.getPackageOf(typeElement); if (pkg.isUnnamed()) {
    ...
    public static ClassName getType(String className) {
        return ClassName.get(className.substring(0, className.lastIndexOf(".")), className.substring(className.lastIndexOf(".") + 1, className.length()));
    }
}
