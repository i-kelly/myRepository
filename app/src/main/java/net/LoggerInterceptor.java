    ...
    private void logForRequest(Request request) {
        try {
            ...
            if(requestBody instanceof CountingRequestBody) {
                FormBody body = ((FormBody)(((CountingRequestBody) requestBody).delegate));
                if (body.size() > 0) {
                    //封装参数
    ...
    private boolean isText(MediaType mediaType) {
        ...
        if (mediaType.subtype() != null) {
            if (mediaType.subtype()
                         .equals("json") || mediaType.subtype()
                                                     .equals("xml") || mediaType.subtype()
                                                                                .equals("html") || mediaType.subtype()
                                                                                                            .equals("webviewhtml")) {
                return true;
            }
        }
